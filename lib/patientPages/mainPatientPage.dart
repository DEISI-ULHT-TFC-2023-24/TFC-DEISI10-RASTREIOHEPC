    ...
    return Scaffold(
      appBar: AppBar(
        title: Column(
          ...
          children: [
            Text(widget.patient.getName()+', ${widget.patient.getAge()}'), // Display patient's name
          ],
        ),
      ),
      body: IndexedStack(
        index: _selectedIndex,
        children: patientPages.map((page) {
          if (page.title == 'Main Patient Page') {
            // Return MainPatientPage widget with the provided patient
            return MainPatientPage(patient: widget.patient); // Pass the patient object to each MainPatientPage instance
          } else {
            return page.widget;
          }
        }).toList(),
